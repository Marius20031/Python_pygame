...
# Draw boats
def draw_boats_mouse():
    for i, (boat_x, boat_y) in enumerate(boats):
        color = BLUE if selected == i else WHITE
        pygame.draw.rect(screen, color, (boat_x, boat_y, boat_width, boat_height))
...
# Boat dimensions
boat_width = 3 * cell_size
boat_height = 2 * cell_size
...
def run_game():
    ...
    while running:
        for event in pygame.event.get():
            draw_boats_mouse()  # Draw the board
            #draw_board()
            ...
            if event.type == pygame.QUIT:
                ...
                for i, (boat_x, boat_y) in enumerate(boats):
                    if boat_x <= mouse_x <= boat_x + boat_width and boat_y <= mouse_y <= boat_y + boat_height:
                        selected = i
